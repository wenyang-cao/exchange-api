...
@Operation(
  ...
  def getUserApiKeyById(@Parameter(hidden = true)identity: Identity2, 
    ...
    val query = for {
      keyRow <- ApiKeysTQ.getById(keyid).result.headOption
      ownerStrOpt <- ApiKeysTQ.getOrgAndUsernameByKeyId(keyid)
    } yield (keyRow, ownerStrOpt)

  db.run(query.transactionally).map {
    case (Some(keyRow: ApiKeyRow), Some(ownerStr: String)) =>
    ...
    path(Segment) { keyid =>
      get {
        exchAuth(TUser(s"$organization/$username"), Access.READ, validIdentity = identity) { _ =>
          getUserApiKeyById(identity, organization, username, UUID.fromString(keyid))
        }
      } ~
      delete {
        exchAuth(TUser(s"$organization/$username"), Access.WRITE, validIdentity = identity) { _ =>
          deleteUserApiKey(identity, organization, username, UUID.fromString(keyid))
        }
      }
    }
  }
