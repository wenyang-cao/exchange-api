...
lazy val root = (project in file("."))
  .settings(
    ...
    dockerCommands           := Seq(Cmd("FROM", dockerBaseImage.value ++ " AS stage0"),
                                    ...
                                    Cmd("RUN", "mkdir -p /run/user/" ++ (Docker / daemonUserUid).value.get ++ " && microdnf update -y --nodocs --refresh 1>/dev/null 2>&1 && microdnf install -y --nodocs shadow-utils gettext java-17-openjdk openssl 1>/dev/null 2>&1 && microdnf clean all"),
                                    Cmd("USER", "root"),
